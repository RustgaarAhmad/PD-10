        ...
        if (isBulletActive[x] == true)
		 {
            ...
            if (bulletX[x] >= eX1 && bulletX[x] <= eX1 + 2 && bulletY[x] >= eY1 && bulletY[x] <= eY1 + 1) 
			{
                eraseBullet(bulletX[x], bulletY[x]);
                makeBulletInactive(x);
                enemy1Health--; // Decrease health
                if (enemy1Health <= 0)
				 {
                    eraseEnemy1();
                    eX1 = -1; eY1 = -1;
					enemy1Health--; // Mark Enemy1 as inactive
            ...
            if (bulletX[x] >= eX2 && bulletX[x] <= eX2 + 2 && bulletY[x] >= eY2 && bulletY[x] <= eY2 + 1)
			 {
                eraseBullet(bulletX[x], bulletY[x]);
                makeBulletInactive(x);
                enemy2Health--; // Decrease health
                if (enemy2Health <= 0) 
				{
                    eraseEnemy2();
                    eX2 = -1; eY2 = -1; 
					enemy2Health--;// Mark Enemy2 as inactive
                    gamescore += 150; // Different score for Enemy2
                }
                continue;
            }

            // Check for collision with Enemy3
            if (bulletX[x] >= eX3 && bulletX[x] <= eX3 + 2 && bulletY[x] >= eY3 && bulletY[x] <= eY3 + 1) 
			{
                ...
                if (enemy3Health <= 0) {
                    ...
                    eX3 = -1; eY3 = -1;
					enemy3Health--;
                    gamescore += 200; // Different score for Enemy3
                }
                continue;
            }
...
void moveEnemy1() 
{
    ...
    eX1 = eX1 + 1;
    if (eX1 + 4 == 100)
	 { // Reset position if boundary reached
        eX1 = 2;
    }
    printEnemy1();

    // Randomly shoot vertically
    if (rand() % 10 == 0)
	 { 
...
void moveEnemy2() 
{
    ...
    eraseEnemy2();
    eX2 = eX2 + 1; // Horizontal movement
    if (eX2 + 4 == 100) { // Reset position if boundary reached
        eX2 = 5;
    }
    printEnemy2();
...
void moveEnemy3() 
{
    ...
    eraseEnemy3();
    eY3 = eY3 + 1; // Vertical movement
    if (eY3 + 2 == 30) { // Reset position if boundary reached
        eY3 = 10;
    }
    printEnemy3();
...
//             if (nextChar != ' ') { // Collision detection
//                 gotoxy(enemyBulletX[i], enemyBulletY[i]);
//                 cout << " "; // Erase bullet
//                 isEnemyBulletActive[i] = false; // Deactivate bullet

//                 if (nextChar == '0' || nextChar == '(' || nextChar == ')') {
//                     health--; // Reduce health on player collision
//                 }
//             } else {
//                 gotoxy(enemyBulletX[i], enemyBulletY[i]);
//                 cout << " "; // Erase previous bullet position
//                 enemyBulletY[i]++; // Move bullet downward
//                 gotoxy(enemyBulletX[i], enemyBulletY[i]);
//                 cout << "^"; // Draw bullet
//             }
//         }
    ...
    for (int i = 0; i < enemyBulletCount; i++) 
	{
        if (isEnemyBulletActive[i]) 
		{
            ...
            if ((enemyBulletY[i] + 1 == pY || enemyBulletY[i] + 1 == pY + 1) &&
                ...
                if (health <= 0) {
                    playerLose(); // Call lose condition
                }
                continue;
            }

            // Check if bullet hits an obstacle or boundary
            if (nextChar != ' ')
			 {
                gotoxy(enemyBulletX[i], enemyBulletY[i]);
                cout << " "; // Erase bullet
                ...
                enemyBulletY[i]++; // Update Y position
                gotoxy(enemyBulletX[i], enemyBulletY[i]);
                cout << "^"; // Draw bullet
            }
        }
...
void generateVerticalEnemyBullet(int eX, int eY)
 {
    enemyBulletX[enemyBulletCount] = eX;       // Same X as the enemy
    enemyBulletY[enemyBulletCount] = eY + 1;  // Start below the enemy
    isEnemyBulletActive[enemyBulletCount] = true;
    gotoxy(eX, eY + 1);
    cout << "^"; // Bullet visual
    enemyBulletCount++;
...
void generateSecondEnemyBullet(int eX, int eY) 
{
    enemyBulletX[enemyBulletCount] = eX + 1; // Centered under the enemy
    enemyBulletY[enemyBulletCount] = eY + 2; // Start below the enemy
    isEnemyBulletActive[enemyBulletCount] = true;
    gotoxy(eX + 1, eY + 2);
    cout << "^"; // Bullet visual
    enemyBulletCount++;
